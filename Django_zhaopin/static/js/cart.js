...
function cartAddMin(){
	...
	$('.mz-adder-add').click(function(){
		...
		var gstore =$(this).parents('.cart-product-number-adder').find('input[name=gstore]').val();
		console.log(gstore)
		gnum ++;
		if(gnum<=gstore){
			$(this).removeClass('disabled');
			$(this).parents('.cart-product-number-adder').find('.cart-product-number-max').text('');
		}else{
			gnum=gstore;
			$(this).parents('.cart-product-number-adder').find('.cart-product-number-max').text('库存不足,请重新选择!');
